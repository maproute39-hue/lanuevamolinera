  ...
  if (filters?.categoria) {
    const categoriaSegura = filters.categoria.replace(/"/g, '\\"');
    conditions.push(`category = "${categoriaSegura}"`);
  }

  if (filters?.capacidadMin != null) {
    conditions.push(`capacity_max >= ${filters.capacidadMin}`);
  }

  if (filters?.precioMax != null) {
    conditions.push(`price_from <= ${filters.precioMax}`);
  }
  ...
  return records.map((record: any) => this.mapServicio(record));
}
  }